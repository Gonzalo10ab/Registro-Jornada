...
<body>
    ...
    <div>
            ...
            <tbody>
                ...
                @foreach ($usuario->registros as $registro)
                    @php
                        ...
                        $totalHoras += is_numeric($horasTrabajadas) ? $horasTrabajadas : 0;
                    @endphp
                    <tr>
                        ...
                        <td>
                            @if (is_numeric($horasTrabajadas))
                                {{ floor($horasTrabajadas) }}h {{ round(($horasTrabajadas - floor($horasTrabajadas)) * 60) }}min
                            @else
                                ---
                            @endif
                        </td>
        ...
        <p class="total">Total de Horas Trabajadas: {{ floor($totalHoras) }}h {{ round(($totalHoras - floor($totalHoras)) * 60) }}min</p>
    </div>
